...
function drawSummary(report: Report, doc: jsPDF, logo: string, startY: number) {
  ...
  const drawHeader = () => {
    ...
    headers.forEach((h, i) => {
      ...
      doc.text(h, tx, y + 4.6, { align, maxWidth: widths[i] - 2.4 });
      x += widths[i];
    });
    doc.setTextColor(...DARK);
    y += 7;
  ...
  rows.forEach((r) => {
    ...
    values.forEach((_v, i) => {
      doc.setDrawColor(...BORDER);
      if (r.subtotal || r.grand) doc.setFillColor(...SUBTOTAL_BG);
...
async function mergePdfAttachments(baseBytes: ArrayBuffer, report: Report) {
  /*
    Mantém a funcionalidade já existente para anexos PDF.
    Cada página do anexo é incorporada ao final do documento principal.
  */
  ...
  for (const a of report.anexos) {
    ...
    try {
      ...
      // Um PDF protegido por senha ou corrompido não bloqueia o restante do relatório.
